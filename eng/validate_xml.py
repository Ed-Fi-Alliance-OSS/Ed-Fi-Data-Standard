...
class XMLValidator:
    ...
    DESCRIPTOR_PATTERN = re.compile(r'uri://(?P<namespace>[^/]+)/(?P<descriptor>[^#]+)#(?P<codeValue>[^<]+)')
    ...
    def validate_schema(self, xml_file: Path) -> None:
        ...
        try:
            ...
            self.add_error(ValidationError(
                str(xml_file), 1, f"Error processing file: {str(e)}"
            ))
    ...
    def validate_descriptors(self, xml_file: Path) -> None:
        ...
        try:
            ...
            self.add_error(ValidationError(
                str(xml_file), 1, f"Error reading file for descriptor validation: {str(e)}"
            ))
    
    def _validate_descriptor_reference(self, xml_file: Path, line_number: int, 
                                     namespace: str, descriptor: str, code_value: str) -> None:
        ...
        if not descriptor_file.exists():
            self.add_error(ValidationError(
                str(xml_file), line_number, f"No matching descriptor file for {descriptor}"
            ))
            return
        ...
        if descriptor_key not in self._descriptor_cache[descriptor]:
            self.add_error(ValidationError(
                str(xml_file), line_number, f"No matching code value {code_value} for {descriptor}"
            ))
    ...
    def _load_descriptor_data(self, descriptor_file: Path, descriptor: str) -> None:
        ...
        try:
            ...
            print(f"Warning: Error loading descriptor file {descriptor_file}: {e}", file=sys.stderr)
            self._descriptor_cache[descriptor] = set()
    
    def validate_all(self) -> bool:
        """Validate all XML files in the samples directory."""
        if not self.samples_dir.exists():
            print(f"Error: Samples directory {self.samples_dir} does not exist", file=sys.stderr)
            return False
        
        if not self.schemas_dir.exists():
            print(f"Error: Schemas directory {self.schemas_dir} does not exist", file=sys.stderr)
            return False
        
        if not self.descriptors_dir.exists():
            print(f"Error: Descriptors directory {self.descriptors_dir} does not exist", file=sys.stderr)
            return False
